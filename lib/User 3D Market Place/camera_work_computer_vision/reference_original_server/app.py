...
def analyze_image():
    ...
    if used_browser_pose:
        msg = 'Image processed using live browser pose (33 landmarks)'
    elif HAS_MEDIAPIPE:
...
if __name__ == '__main__':
    ...
    print(f"\n--- SERVER RUNNING ---")
    if use_plain_http:
        print("SMARTFITAO_HTTP=1  ->  HTTP (no SSL). Desktop test: http://127.0.0.1:5000")
    else:
        print(f"Scan the QR code or visit https://{get_local_ip()}:5000 on your mobile device.")
    print(f"----------------------\n")
    if use_plain_http:
